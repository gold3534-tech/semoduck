...
async function getData() {
  ...
  const [itemsResult, galleriesResult] = await Promise.all([
    ...
    admin.from("galleries").select("id,name,slug,description,category,thumbnail_url,follower_count,post_count").order("name")
  ]);
...
export default async function MarketPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <MarketBoard initialItems={items} galleries={galleries} />
    </div>
  );
}
