...
export default async function MarketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: item }, { data: inquiries }] = await Promise.all([
    admin
    .from("market_items")
    .select("id,title,description,trade_type,status,price,region,image_url,created_at,profiles(nickname,email),galleries(name,slug)")
    .eq("id", id)
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[28rem_1fr]">
      ...
      <div className="space-y-5">
        ...
        <Card>
          <h2 className="font-black">거래 안내</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">세모덕은 결제와 배송을 대행하지 않습니다. 거래 전 상품 상태, 가격, 배송 방식, 상대방 정보를 직접 확인해주세요.</p>
        </Card>
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {(inquiries ?? []).map((inquiry) => {
              ...
              return (
                <div key={inquiry.id} className="rounded-lg bg-cloud p-4">
                  <p className="font-black">{writer?.nickname ?? writer?.email ?? "회원"}</p>
                  <p className="mt-2 text-slate-700">{inquiry.content}</p>
