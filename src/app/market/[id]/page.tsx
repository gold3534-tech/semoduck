...
function relatedTermsForMarket(item: { title: string; description?: string | null; galleries?: { name?: string | null; slug?: string | null } | null }) {
  return [...new Set([item.title, item.galleries?.name, item.galleries?.slug, ...(item.description ?? "").split(/\s+/).slice(0, 4)].filter(Boolean) as string[])].slice(0, 6);
}
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-slate-500">
                <span className="inline-flex items-center gap-1.5">
                  <UserRound size={15} /> {profile?.nickname ?? profile?.email ?? "회원"}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <MapPin size={15} /> {item.region || "거래 지역 협의"}
                </span>
              ...
              <div className="grid gap-2 text-sm font-bold text-slate-600 sm:grid-cols-2">
                <p className="rounded-xl bg-[#fbf4ff] px-3 py-2">
                  거래 유형 <span className="ml-2 text-[#3a285f]">{tradeTypeLabel(item.trade_type)}</span>
                </p>
                <p className="rounded-xl bg-[#fbf4ff] px-3 py-2">
                  상품 상태 <span className="ml-2 text-[#3a285f]">{item.status === "completed" ? "거래완료" : "거래 가능"}</span>
                </p>
                <p className="rounded-xl bg-[#fbf4ff] px-3 py-2">
                  배송 방법 <span className="ml-2 text-[#3a285f]">택배/직거래 협의</span>
                </p>
                <p className="rounded-xl bg-[#fbf4ff] px-3 py-2">
                  거래 장소 <span className="ml-2 text-[#3a285f]">{item.region || "협의 가능"}</span>
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {!isOwner ? (
                  <Button className="min-h-9 rounded-xl px-4 py-1.5 text-xs">
                    <Send size={15} /> 거래톡 보내기
                  </Button>
                ) : null}
                <Button variant="secondary" className="min-h-9 rounded-xl px-4 py-1.5 text-xs">
                  <Heart size={15} /> 찜하기
                </Button>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Card className="p-4">
              ...
              <dl className="mt-3 divide-y divide-[#f1dbe8] text-sm font-bold">
                ...
                <div className="grid grid-cols-[7rem_1fr] py-2">
                  ...
                  <dd className="text-slate-600">{item.region || "협의 가능"}</dd>
                </div>
                <div className="grid grid-cols-[7rem_1fr] py-2">
                  <dt className="text-[#6f4ab4]">결제 방식</dt>
                  <dd className="text-slate-600">안전결제 또는 계좌이체 협의</dd>
                </div>
              </dl>
            </Card>
            ...
            <Card className="p-4">
              ...
              <Link href="/market" className="mt-4 inline-flex min-h-8 w-full items-center justify-center rounded-xl bg-white text-xs font-black text-[#6f4ab4] ring-1 ring-[#ead8f4]">
                판매자 다른 상품 보기
              </Link>
            </Card>
          </div>
          ...
          <Card className="flex items-center gap-3 border-[#ffd8aa] bg-[#fff8ef] p-4">
            ...
            <p className="text-sm font-bold text-[#6b4b2c]">직거래와 택배 거래 전 상품 상태와 거래 조건을 꼭 확인해 주세요.</p>
          </Card>
        </div>

        <RelatedPostList title="이 굿즈 관련 게시글" posts={(relatedPosts ?? []) as RelatedPostItem[]} />
      </div>
    </div>
  );
}
