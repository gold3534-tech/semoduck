...
async function requirePostAccess(id: string) {
  ...
  if (!user || !authClient) return { error: NextResponse.json({ error: "로그인이 필요합니다." }, { status: 401 }) };
  ...
  return { db, user, isOwner: post?.user_id === user.id, isAdmin: isAdminEmail(user.email) };
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await access.db.from("posts").update({ ...body, updated_at: new Date().toISOString() }).eq("id", id);
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await access.db.from("posts").update({ is_deleted: true, updated_at: new Date().toISOString() }).eq("id", id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true });
  } catch (error) {
