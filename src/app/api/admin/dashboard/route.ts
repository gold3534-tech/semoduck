...
async function requireAdmin() {
  ...
  try {
    return { admin: createAdminSupabaseClient(), serviceRoleReady: true };
  } catch {
    if (!authClient) {
      return { error: NextResponse.json({ error: "서버 설정을 확인해 주세요. 관리자 키가 누락되었습니다." }, { status: 500 }) };
    }
    return { admin: authClient, serviceRoleReady: false };
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      serviceRoleReady: session.serviceRoleReady,
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "관리자 데이터를 불러오지 못했습니다." }, { status: 500 });
  }
}
