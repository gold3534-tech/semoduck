...
export function SignupForm() {
  ...
  return (
    <section className="w-full max-w-xl rounded-[2rem] border border-[#ead8f4] bg-white/86 px-8 py-8 shadow-[0_22px_70px_rgba(126,80,178,0.13)] backdrop-blur md:px-12">
      <div className="text-center">
        <h1 className="text-3xl font-black text-[#6f4ab4] md:text-4xl">
          회원가입 <span className="text-[#ff6f9b]">♡</span>
        </h1>
        <p className="mt-2 text-sm font-bold text-[#70657f] md:text-base">아이디와 비밀번호로 세모덕 계정을 만들어보세요.</p>
      </div>
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <label className="flex min-h-14 items-center gap-3 rounded-2xl border border-[#e5ddee] bg-white px-4 text-[#9b63d6] shadow-sm focus-within:border-[#b984e7]">
          <UserRound size={23} />
          <input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-bold text-[#2f2352] outline-none placeholder:text-[#a9a0b8] md:text-base" placeholder="아이디로 사용할 이메일을 입력하세요" />
        </label>

        <label className="flex min-h-14 items-center gap-3 rounded-2xl border border-[#e5ddee] bg-white px-4 text-[#9b63d6] shadow-sm focus-within:border-[#b984e7]">
          <LockKeyhole size={23} />
          <input type={showPassword ? "text" : "password"} required minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-bold text-[#2f2352] outline-none placeholder:text-[#a9a0b8] md:text-base" placeholder="비밀번호를 입력하세요" />
        ...
        <label className="flex min-h-14 items-center gap-3 rounded-2xl border border-[#e5ddee] bg-white px-4 text-[#9b63d6] shadow-sm focus-within:border-[#b984e7]">
          <LockKeyhole size={23} />
          <input type={showPassword ? "text" : "password"} required minLength={6} value={passwordConfirm} onChange={(event) => setPasswordConfirm(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-bold text-[#2f2352] outline-none placeholder:text-[#a9a0b8] md:text-base" placeholder="비밀번호를 한 번 더 입력하세요" />
        </label>
