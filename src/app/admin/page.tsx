...
type LinkSubmission = { id: string; title: string; url: string; source: string; status: string; created_at: string };
...
type SuggestionGalleryDraft = {
  slug?: string;
  category?: string;
  thumbnailUrl?: string;
  officialSiteUrl?: string;
  officialShopUrl?: string;
  officialShopLabel?: string;
};
...
export default function AdminPage() {
  ...
  const [suggestionGalleryDrafts, setSuggestionGalleryDrafts] = useState<Record<string, SuggestionGalleryDraft>>({});
  ...
  async function load() {
    ...
    const officialResponse = await fetch("/api/admin/official-products", { cache: "no-store" });
    if (officialResponse.ok) {
      const officialData = (await officialResponse.json()) as { items: OfficialProductCandidate[] };
      setOfficialCandidates(officialData.items);
    }
  }
  ...
  async function collectOfficialProducts() {
    ...
    setCollectMessage("");
    const response = await fetch("/api/admin/official-products", { method: "PUT" });
    const result = (await response.json()) as { items?: OfficialProductCandidate[]; skipped?: Array<{ gallerySlug: string; mallName: string; reason: string }>; error?: string };
    setSavingId(null);
    ...
    setCollectMessage(`수집 후보 ${result.items.length}개 · 제외/대체 ${result.skipped?.length ?? 0}건`);
  }

  async function importOfficialProduct(candidate: OfficialProductCandidate) {
    setSavingId(`official-${candidate.key}`);
    const response = await fetch("/api/admin/official-products", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ key: candidate.key, candidate: { ...candidate, source: candidate.source ?? "official_shop" } })
    });
    setSavingId(null);
    if (!response.ok) {
      const error = (await response.json()) as { error?: string };
  ...
  async function createGallery(fromSuggestion?: Suggestion) {
    const suggestionDraft = fromSuggestion ? suggestionGalleryDrafts[fromSuggestion.id] : undefined;
    const form = fromSuggestion
      ? {
          name: fromSuggestion.requested_gallery_name || galleryForm.name,
          slug: suggestionDraft?.slug || "",
          category: suggestionDraft?.category || fromSuggestion.requested_gallery_category || galleryForm.category || "기타",
          description: "",
          thumbnailUrl: suggestionDraft?.thumbnailUrl || galleryForm.thumbnailUrl,
          officialSiteUrl: suggestionDraft?.officialSiteUrl || "",
          officialShopUrl: suggestionDraft?.officialShopUrl || "",
          officialShopLabel: suggestionDraft?.officialShopLabel || ""
        }
      : galleryForm;

    if (!form.slug.trim()) {
      alert("갤러리 주소를 입력해주세요.");
      return;
    }
  ...
  async function saveThumbnail(gallery: Gallery) {
    ...
    const response = await fetch(`/api/admin/galleries/${gallery.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ thumbnailUrl: thumbnailDrafts[gallery.id] ?? gallery.thumbnail_url ?? "" })
    });
    setSavingId(null);
    if (!response.ok) {
    ...
    <div className="space-y-6">
      ...
      {activeTab === "suggestions" ? (
        <div className="space-y-4">
          {data.suggestionSetupMissing ? (
            <Card className="bg-amber-50 text-amber-800">
              건의함 테이블이 아직 없습니다. Supabase SQL Editor에서 <span className="font-black">supabase/admin-suggestions.sql</span>을 실행하면 건의 저장과 관리자 확인이 켜집니다.
            </Card>
          ) : null}
          <Card>
                    ...
                    <div className="grid gap-2">
                      {suggestion.type === "gallery_request" ? (
                        <div className="grid gap-2 sm:grid-cols-3">
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.slug ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], slug: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="주소 slug"
                          />
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.category ?? suggestion.requested_gallery_category ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], category: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="카테고리"
                          />
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.thumbnailUrl ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], thumbnailUrl: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="이미지 URL"
                          />
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.officialSiteUrl ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], officialSiteUrl: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="공식 SNS URL"
                          />
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.officialShopUrl ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], officialShopUrl: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="공식굿즈샵 URL"
                          />
                          <input
                            value={suggestionGalleryDrafts[suggestion.id]?.officialShopLabel ?? ""}
                            onChange={(event) => setSuggestionGalleryDrafts((current) => ({ ...current, [suggestion.id]: { ...current[suggestion.id], officialShopLabel: event.target.value } }))}
                            className="min-h-10 rounded-lg border px-3 text-sm font-bold"
                            placeholder="공식샵 이름"
                          />
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                      {suggestion.type === "gallery_request" ? <Button variant="secondary" onClick={() => createGallery(suggestion)} disabled={savingId === `gallery-${suggestion.id}`}>갤러리 추가</Button> : null}
                      <Button variant="secondary" onClick={() => resolveSuggestion(suggestion.id, "resolve", "확인 완료")}>처리 완료</Button>
                      <Button variant="danger" onClick={() => resolveSuggestion(suggestion.id, "reject", "반려")}>반려</Button>
                      </div>
                    </div>
                  ) : null}
                </div>
          ...
          <Card>
            ...
            <div className="mt-4 grid gap-3">
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <input value={galleryForm.slug} onChange={(event) => setGalleryForm({ ...galleryForm, slug: event.target.value })} className="min-h-10 rounded-lg border px-3" placeholder="주소 slug(필수)" />
              ...
              <textarea value={galleryForm.description} onChange={(event) => setGalleryForm({ ...galleryForm, description: event.target.value })} className="min-h-20 rounded-lg border p-3" placeholder="갤러리 설명(선택, 나중에 로컬 LLM으로 생성)" />
              <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                ...
                <Button onClick={() => createGallery()} disabled={savingId === "gallery" || !galleryForm.name || !galleryForm.slug || !galleryForm.category}>갤러리 추가</Button>
              </div>
              <div className="grid gap-3 md:grid-cols-3">
                <input value={galleryForm.officialSiteUrl} onChange={(event) => setGalleryForm({ ...galleryForm, officialSiteUrl: event.target.value })} className="min-h-10 rounded-lg border px-3" placeholder="공식 SNS URL" />
                <input value={galleryForm.officialShopUrl} onChange={(event) => setGalleryForm({ ...galleryForm, officialShopUrl: event.target.value })} className="min-h-10 rounded-lg border px-3" placeholder="공식굿즈샵 URL" />
                <input value={galleryForm.officialShopLabel} onChange={(event) => setGalleryForm({ ...galleryForm, officialShopLabel: event.target.value })} className="min-h-10 rounded-lg border px-3" placeholder="공식샵 이름" />
              </div>
            </div>
          </Card>
        </div>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-[22rem_1fr]">
            ...
            {selectedGroup ? (
              <div className="space-y-4">
                ...
                {selectedGroup.reports.map((report) => <div key={report.id} className="rounded-lg border border-slate-100 p-3"><p className="font-black">{report.category ?? "기타"}</p><p className="mt-1 text-sm text-slate-700">{report.detail ?? report.reason}</p></div>)}
        ...
        <div className="space-y-4">
          <Card>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-black">공식몰 상품 후보</h2>
                <p className="mt-2 text-sm font-bold text-slate-500">산리오는 네이버 공식 브랜드스토어 검색 링크로 두고, 나머지는 공식몰에서 상품 후보를 가져옵니다. 등록하면 굿즈 검색에 공식 후보로 우선 노출됩니다.</p>
                {collectMessage ? <p className="mt-2 text-sm font-black text-mint">{collectMessage}</p> : null}
              </div>
              <Button variant="secondary" onClick={collectOfficialProducts} disabled={savingId === "official-collect"}>
                {savingId === "official-collect" ? <Loader2 size={16} className="animate-spin" /> : null}
                공식몰 후보 수집
              </Button>
            </div>
            <div className="mt-4 divide-y divide-slate-100">
              {officialCandidates.map((candidate) => (
                <div key={candidate.key} className="grid gap-3 py-4 md:grid-cols-[1fr_auto] md:items-center">
                  <div>
                    <div className="flex flex-wrap gap-2">
                      <Badge tone="mint">공식몰</Badge>
                      <Badge tone="gray">{candidate.gallerySlug}</Badge>
                      {candidate.availabilityLabel ? <Badge tone="sun">{candidate.availabilityLabel}</Badge> : null}
                    </div>
                    <p className="mt-2 font-black">{candidate.title}</p>
                    <a href={candidate.url} target="_blank" rel="noopener noreferrer" className="mt-1 block text-sm font-bold text-slate-500 hover:text-berry">
                      {candidate.mallName}
                    </a>
                    {candidate.price ? <p className="mt-1 text-sm font-black text-ink">{candidate.price.toLocaleString("ko-KR")}원</p> : null}
                  </div>
                  <Button variant="secondary" onClick={() => importOfficialProduct(candidate)} disabled={savingId === `official-${candidate.key}`}>
                    공식 상품 등록
                  </Button>
                </div>
              ))}
              {!officialCandidates.length && <p className="py-4 text-sm font-bold text-slate-500">등록 가능한 공식몰 후보가 없습니다.</p>}
            </div>
          </Card>
          <Card>
            ...
            <div className="mt-4 grid gap-3">
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                ...
                <label className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg bg-white px-4 text-sm font-bold ring-1 ring-slate-200"><Upload size={16} /> 이미지 업로드<input type="file" accept="image/*" className="hidden" onChange={(event) => event.target.files?.[0] && uploadProductImage(event.target.files[0])} /></label>
              </div>
              <div className="grid gap-3 md:grid-cols-4">
                <select value={productForm.source} onChange={(event) => setProductForm({ ...productForm, source: event.target.value as ProductForm["source"] })} className="min-h-10 rounded-lg border px-3">
                  <option value="official_shop">공식몰</option><option value="naver_shopping">네이버</option><option value="coupang">쿠팡</option><option value="user_submission">유저 제보</option><option value="external_search">외부 검색</option>
                </select>
          ...
          <Card>
            ...
            <div className="mt-4 divide-y divide-slate-100">
              {data.products.map((product) => <div key={product.id} className="grid gap-3 py-4 md:grid-cols-[1fr_auto] md:items-center"><div><p className="font-black">{product.title}</p><p className="mt-1 text-sm font-bold text-slate-500">{product.brand || "브랜드 없음"} · {product.category || "카테고리 없음"} · 신고 {product.report_count ?? 0}건</p></div><Button variant="danger" onClick={() => deleteProduct(product.id)}>삭제</Button></div>)}
            </div>
          </Card>
          <Card>
            <h2 className="text-xl font-black">상품 링크 제보</h2>
            <div className="mt-4 divide-y divide-slate-100">
              {data.linkSubmissions.map((submission) => <div key={submission.id} className="grid gap-3 py-4 md:grid-cols-[1fr_auto] md:items-center"><div><a href={submission.url} target="_blank" className="font-black hover:text-berry">{submission.title}</a><p className="mt-1 text-sm font-bold text-slate-500">{submission.source} · {submission.status} · {formatDateTime(submission.created_at)}</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => resolveLinkSubmission(submission.id, "approve")}><CheckCircle2 size={16} /> 승인</Button><Button variant="danger" onClick={() => resolveLinkSubmission(submission.id, "reject")}><XCircle size={16} /> 거절</Button></div></div>)}
            </div>
          </Card>
          <Card>
            <h2 className="text-xl font-black">등록된 판매 링크</h2>
            <div className="mt-4 divide-y divide-slate-100">
              {data.productOffers.map((offer) => <a key={offer.id} href={offer.url} target="_blank" className="grid gap-2 py-3 text-sm hover:text-berry"><p className="font-black">{offer.products?.title ?? offer.mall_name}</p><p className="font-bold text-slate-500">{sourceLabel(offer.source)} · {offer.mall_name} · {offer.price.toLocaleString("ko-KR")}원</p></a>)}
            </div>
          </Card>
        </div>
      ) : null}
