...
export function GalleryCard({ gallery, compact = false, rank, followed = false, followBusy = false, onToggleFollow }: GalleryCardProps) {
  ...
  const followButton = onToggleFollow ? (
    <button
      ...
      aria-label={followed ? "자주가는 갤러리 해제" : "자주가는 갤러리 추가"}
      title={followed ? "자주가는 갤러리 해제" : "자주가는 갤러리 추가"}
      className={`absolute right-3 top-3 z-20 grid h-9 w-9 place-items-center rounded-full shadow-sm ring-1 transition ${
        followed ? "bg-[#fff2bd] text-[#3a285f] ring-[#ffe08a]" : "bg-white text-[#6f4ab4] ring-[#ead8f4] hover:bg-[#fff1f7]"
      } disabled:cursor-wait disabled:opacity-70`}
    >
      <Star size={16} className={followed ? "fill-current" : ""} />
    </button>
  ) : null;
